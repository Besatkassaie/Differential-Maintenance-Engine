...
public class Person_ {
    private static Map<String, Person_> personMap = new HashMap<String, Person_>();
     ...
     */
    private Person_(String name, String urlpt) {
        this.name = name;
        this.urlpt = urlpt;
     ...
     */
    static public Person_ create(String name, String urlpt) {
        Person_ p;
        p = searchPerson(name);
        if (p == null)
            p = new Person_(name, urlpt);
        return p;
    }
     ...
     */
    static public Person_ searchPerson(String name) {
    ...
    private void loadCoauthors() {
        ...
        coauthors = new Person_[plist.size()];
        coauthors = plist.toArray(coauthors);
        coauthorsLoaded = true;
    }

    public Person_[] getCoauthors() {
        if (!coauthorsLoaded) {
            loadCoauthors();
        }
        return coauthors;
    ...
    static public void resetAllLabels() {
        Iterator<Person_> i = personMap.values().iterator();
        while (i.hasNext()) {
            Person_ p = i.next();
            p.labelvalid = false;
            p.label = 0;
    ...
    private Publication_ publications[];
    private Publication_ personRecord;

    static private SAXParser publParser;
    static private PublConfigHandler publHandler;
    static private List<Publication_> publlist = new ArrayList<Publication_>();
    static private Publication_ hp;
    ...
    static private class PublConfigHandler extends DefaultHandler {
        ...
        public void endElement(String namespaceURI, String localName,
            ...
            if (rawName.equals("dblpkey") && Value.length() > 0) {
                Publication_ p = Publication_.create(Value);
                if (insideHp)
                    hp = p;
                else
                    publlist.add(p);
    ...
    private void loadPublications() {
        ...
        publications = new Publication_[publlist.size()];
    ...
    public Publication_[] getPublications() {
        if (!publLoaded) {
            loadPublications();
        }
        return publications;
    ...
    public Publication_ getPersonRecord() {
        if (!publLoaded) {
            loadPublications();
        }
        return personRecord;
